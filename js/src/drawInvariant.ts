...
const canvas = document.getElementById("draw-invariant") as HTMLCanvasElement;
const ctx = canvas.getContext("2d", { willReadFrequently: true })!;

const dragCanvas = document.getElementById("drag-invariant") as HTMLCanvasElement;
const dragCtx = dragCanvas.getContext("2d", { willReadFrequently: true })!;

const output = document.getElementById("output-invariant")!;
const probsContainer = document.getElementById("probabilities-invariant")!;
...
async function getSession() {
  ...
  const modelUrl = new URL("./assets/mnist_invariant_cnn.onnx", import.meta.url).href;
...
document.getElementById("clearBtn-invariant")!.addEventListener("click", () => {
  resetDrawCanvas();
  // patchLeft = 14;
  // patchTop = 14;
  renderDragPreview();
